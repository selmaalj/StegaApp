...
parser.add_argument('--image', type=str, required=True)  # Changed 'video' to 'image'
parser.add_argument('--secret_size', type=int, default=100)
parser.add_argument('--save_image', type=str, default=None)  # Changed 'save_video' to 'save_image'
...
def main():
    ...
    mask_im = cv2.resize(np.float32(out_vis_image), (image.shape[1], image.shape[0]))
        ...
        for im_rotation in range(4):
            ...
            if bitflips != -1:
                ...
                try:
                    ...
                    continue
                color = (100,250,100)
                cv2.polylines(image, np.int32([corners]), thickness=6, color=color, isClosed=True)
                font = cv2.FONT_HERSHEY_SIMPLEX
                im = cv2.putText(image, code, tuple((corners[0,:]+np.array([0,-15])).astype(np.int)), font, 1,(0,0,0), 2, cv2.LINE_AA)

    if args.save_image is not None:
        cv2.imwrite(args.save_image, image)  # Save image
    else:
